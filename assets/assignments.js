...
function renderStats(){
  ...
  $('assignmentAverage').textContent=graded.length?Math.round(graded.reduce((n,s)=>n+Number(s.percent ?? (Number(s.score||0)/Math.max(1,Number(s.maxScore||100))*100)),0)/graded.length)+'%':'—';
}
function render(){
  renderStats();
  const arr=filtered();
...
function openAssignment(id){
  ...
  (st==='graded'
    ?'<div class="assignment-feedback"><span>✅</span><div><strong>تم التصحيح • '+Number(s.score||0)+' / '+Number(s.maxScore||a.maxScore||100)+' ('+Number(s.percent ?? Math.round(Number(s.score||0)/Math.max(1,Number(s.maxScore||a.maxScore||100))*100))+'%)</strong><p>'+C.esc(s.feedback||'لا توجد ملاحظات إضافية من المدرس.')+'</p></div></div><div class="assignment-submission-preview"><small>إجابتك</small><p>'+C.esc(s.answer||'—')+'</p>'+(s.link?'<a href="'+C.safeUrl(s.link)+'" target="_blank" rel="noopener">فتح الرابط المرفق</a>':'')+'</div>'
